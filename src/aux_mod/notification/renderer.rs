...
//! - Toast = 2 個 draw 項(bg PNG + item icon)
//! - Float = 1 個 draw 項(EXP/Gold icon,**沒數字**)
//!
...
// 以下三個位址僅供 build_draw_loop_shellcode(舊 codecave 繪圖路徑)使用,
// renderer_install 移除後該路徑只剩單元測試引用,故與函式一同以 cfg(test) 限定。
...
///
/// 舊 codecave 繪圖路徑序列化器;renderer_install 移除後僅單元測試引用,以 cfg(test) 限定。
#[cfg(test)]
...
/// ```
///
/// 舊 codecave 繪圖路徑 shellcode;renderer_install 移除後僅單元測試引用,以 cfg(test) 限定。
#[cfg(test)]

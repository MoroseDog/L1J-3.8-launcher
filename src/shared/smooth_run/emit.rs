...
use super::types::{RunPairMap, RunSlot, SpriteFile};
...
pub fn emit(sf: &SpriteFile, walk_to_run: &RunPairMap) -> (String, VariantInfo) {
    ...
    for (idx, slot) in sid_at_idx.iter_mut().enumerate() {
            ...
            if last_line_per_sprite.get(&sid) == Some(&idx) {
                if let Some(p) = walk_to_run.get(&sid) {
                    ...
                    if let Some(rl) = &p.runl {
                        output.push('\n');
                        output.push_str(&format!("\t{}.walk({})", RunSlot::Left.slot(), rl));
                    }
                    if let Some(rr) = &p.runr {
                        output.push('\n');
                        output.push_str(&format!("\t{}.walk({})", RunSlot::Right.slot(), rr));
                    }
                }
            }
        }

...
use super::types::{Action, Sprite, SpriteFile, INLINE_EXTRA_ACTIONS, WALK_ACTIONS};
    ...
    for (idx, line) in raw_lines.iter().enumerate() {
        ...
        if trimmed.starts_with('#') {
            if let Some(sid) = parse_sprite_id(trimmed) {
                ...
                // 掃描順序 = WALK_ACTIONS 後接 INLINE_EXTRA_ACTIONS,與 legacy 一致
                for &action_num in WALK_ACTIONS.iter().chain(INLINE_EXTRA_ACTIONS) {

...
/// ws2_32 connect 重導向的共用參數 — `connect` 與 `WSAConnect` 兩次 hook 完全共用,
/// 只有匯出名 `func_name` 不同。建一次傳兩次,避免 8 個位置參數重複帶。
struct ConnectRedirect<'a> {
    /// 目標行程 handle
    h: HANDLE,
    /// ws2_32.dll 模組基址
    ws2_base: u32,
    /// 重導向目標 IP(4 bytes,寫進 shellcode)
    ip_bytes: [u8; 4],
    /// 重導向目標 port(big-endian 2 bytes)
    port_be: [u8; 2],
    /// 目標 IP 字串(僅 log 用)
    ip: &'a str,
    /// 目標 port(僅 log 用)
    port: u16,
    /// hover 計數全域位址(shellcode 用)
    hover_count_addr: u32,
}

fn hook_winsock_connect_fn(redirect: &ConnectRedirect, func_name: &str) -> Result<bool> {
    // 解構回同名區域變數 → 下方 body 維持原狀,語義零變動
    let &ConnectRedirect {
        h,
        ws2_base,
        ip_bytes,
...
pub fn hook_connect(h: HANDLE, pid: u32, ip: &str, port: u16, hover_count_addr: u32) -> Result<()> {
    ...
    let result = (|| -> Result<(bool, bool)> {
        ...
        let connect_ok = hook_winsock_connect_fn(&redirect, "connect")?;
        let wsa_ok = hook_winsock_connect_fn(&redirect, "WSAConnect")?;

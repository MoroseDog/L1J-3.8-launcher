...
impl SpellBook {
    /// Build a `name -> packed_skill_id` cache from the live player spell book.
    pub fn build(h: HANDLE) -> Result<Self> {
        let book_ptr = read_u32(h, address::SPELL_BOOK_PTR)
            .with_context(|| format!("read SPELL_BOOK_PTR @ 0x{:08X}", address::SPELL_BOOK_PTR))?;
        if book_ptr == 0 {
            bail!("SPELL_BOOK_PTR is NULL; player is probably not in game");
        }

        let count = read_u32(h, book_ptr + 0x2C)
            .with_context(|| format!("read spell_book.count @ 0x{:08X}", book_ptr + 0x2C))?;
        let array_ptr = read_u32(h, book_ptr + 0x58)
            .with_context(|| format!("read spell_book.array @ 0x{:08X}", book_ptr + 0x58))?;
        if count == 0 || array_ptr == 0 {
            bail!("spell_book is empty: count={count}, array=0x{array_ptr:08X}");
        }
        if count > 1024 {
            bail!("spell_book.count={count} is out of expected range");
        }

        let array_bytes = read_bytes(h, array_ptr, count as usize * 4)
            .with_context(|| format!("read spell array @ 0x{array_ptr:08X}"))?;
    ...
    /// Look up the packed skill id for a learned spell name.
    pub fn lookup(&self, name: &str) -> Option<u32> {
        self.map.get(name).map(|e| e.packed)
    }

    /// Number of unique spell names in the live spell book.
    pub fn unique_names(&self) -> usize {
        self.map.len()
    }

    /// Spell names for diagnostics when lookup misses.
...
mod tests {
    ...
    fn fake_book(book_ptr: u32) -> SpellBook {
        ...
        map.insert("加速術".to_string(), SpellEntry { packed: 0xDEADBEEF });
        SpellBook { map, book_ptr }
    }

    #[test]
    fn lookup_returns_packed_skill_id() {
        let mut map = HashMap::new();
        map.insert("烈炎術".to_string(), SpellEntry { packed: 0x2E });
        let book = SpellBook {
            map,
            book_ptr: 0xAA,
        };

...
pub struct HoverHookResult {
    ...
    pub pid: u32,
    // 此二欄位目前已建構但未被讀取(保留 hook 重裝/卸載資訊);加 allow 避免 dead_code 警告
    #[allow(dead_code)]
...
#[allow(dead_code)]
fn _build_draw_hook_old(cave_addr: u32, draw_addr: u32, _gsr_orig: u32, _surf_mgr: u32) -> Vec<u8> {
...
fn push_scan_loop(sc: &mut Vec<u8>, cave_addr: u32, a_count: u32, a_map: u32) {
    let _gsr_orig = cave_addr + 0x60; // GetSurfResource original trampoline
...
pub fn poll_hover_tick(
    ...
    if cur_count == 0 && tick.is_multiple_of(10) {
    ...
    if !DEBUG_DONE.load(Ordering::Relaxed) {
        ...
        if first_x != 0 {
            for i in 0..n {
                let sx = i32::from_le_bytes(pos_data[i * 8..i * 8 + 4].try_into().unwrap());
                let sy = i32::from_le_bytes(pos_data[i * 8 + 4..i * 8 + 8].try_into().unwrap());
                let _sid = u32::from_le_bytes(map_data[i * 8..i * 8 + 4].try_into().unwrap());
                let _cl = sx * 3 / 2 + off_x;
                let _ct = sy * 3 / 2 + off_y;
                debug_log!(
...
fn scan_element_chain_for_highlight(h: HANDLE, start_elem: u32) {
    ...
    while cur != 0 && cur > 0x10000 && idx < 50 {
        let _src_id = memory::read_u32(h, cur + 0x20).unwrap_or(0);
                ...
                if ptr > 0x10000 {
                    if let Ok(buf) = memory::read_bytes(h, ptr, 0x400) {
                        ...
                        if lower.contains("highlight") {
                            ...
                            if let Some(pos) = lower.find("highlight") {
                                let _start = pos.saturating_sub(20);
                                let _end = (pos + 60).min(text.len());
...
pub fn log_calibration(h: HANDLE, cave_draw: u32, blit_cave: u32, cursor_x: i32, cursor_y: i32) {
    ...
    if let Ok(map_data) = memory::read_bytes(h, cave_draw + OFF_MAP, n * MAP_ENTRY as usize) {
        let _sid = u32::from_le_bytes(map_data[best_idx * 8..best_idx * 8 + 4].try_into().unwrap());
        debug_log!(
    ...
    if let Ok(map_data) = memory::read_bytes(h, cave_draw + OFF_MAP, n * MAP_ENTRY as usize) {
        for i in 0..n {
            ...
            let sy = i32::from_le_bytes(pos_data[i * 8 + 4..i * 8 + 8].try_into().unwrap());
            let _sid = u32::from_le_bytes(map_data[i * 8..i * 8 + 4].try_into().unwrap());
            let _l = sx * 3 / 2 + new_off_x;
            let _t = sy * 3 / 2 + new_off_y;

...
use crate::app::config::WindowMode;
use crate::app::game_launch::{launch_game, PacketEncryptConfig, GAME_EXE};
...
enum UserEvent {
    ...
    AutoUpdatePrompt(crate::app::http::UpdateInfo),
...
fn packet_encrypt_config_from_server(
    aux: &AuxConfig,
    server: &ServerInfo,
) -> std::result::Result<Option<PacketEncryptConfig>, String> {
...
fn try_list_update(launcher_cfg: &launcher::server_list::LauncherConfig, dir: &std::path::Path) {
    ...
    match crate::app::http::http_get(url) {
        Ok(body) => {
            let list_path = dir.join("list.txt");
...
fn check_auto_update(
...
) -> Option<crate::app::http::UpdateInfo> {
    ...
    let body = match crate::app::http::http_get(url) {
        Ok(b) => b,
        Err(e) => {
    ...
    let info = crate::app::http::parse_update_ini(&text, url)?;
...
fn run_auto_update_worker(
    info: crate::app::http::UpdateInfo,
...
fn install_auto_update(
    ...
    let base_url = crate::app::http::base_url_of(&info.source_url);
    log_line!(
    ...
    for (i, (version, entry)) in need.iter().enumerate() {
        let zip_url = crate::app::http::resolve_zip_url(&base_url, entry);
        ...
        let body = crate::app::http::http_get_with_progress(
            &zip_url,
            move |downloaded, total| {
                let cur_pct: u8 = match (downloaded * 100).checked_div(total) {
                    Some(v) => v.min(100) as u8,
                    None => ((downloaded / (64 * 1024)).min(95)) as u8,
                };
...
pub fn run_gui() -> Result<()> {
    ...
    let user_prefs = crate::app::config::UserPrefs::load();
    ...
    let pending_update_shared: Arc<Mutex<Option<crate::app::http::UpdateInfo>>> =
    ...
    let ipc_handler = move |req: Request<String>| {
        ...
        match msg {
            ...
            JsMessage::Launch {
                ...
                // JS 傳來的原始值缺省或無效 → fallback 預設(800x600)
                let mode = window_mode
                    .and_then(WindowMode::from_raw)
                    .unwrap_or(WindowMode::DEFAULT);
                // 順手把當前選擇寫進 launcher.ini,下次啟動沿用
                crate::app::config::UserPrefs {
                    windowed,
                    window_mode: mode,
                }
                .save();
                log_line!("[IPC] 收到 launch 請求 server_idx={server_idx} windowed={windowed} window_mode={}", mode.as_raw());
            ...
            JsMessage::SavePrefs {
                ...
                let mode = WindowMode::from_raw(window_mode).unwrap_or(WindowMode::DEFAULT);
                let prefs = crate::app::config::UserPrefs {
                    windowed,
                    window_mode: mode,
                };
                prefs.save();
        ...
        match event {
            ...
            Event::UserEvent(UserEvent::LaunchGame { ip, port, game_dir, inject_buffer, inject_source_path, packet_encrypt, windowed, window_mode }) => {
                ...
                std::thread::spawn(move || {
                    ...
                    match launch_game(&ip, port, &game_dir, false, inject_buffer, inject_source_path, packet_encrypt, windowed, window_mode, on_started) {
                        Ok(_) => {
...
mod tests {
    ...
    fn packet_encrypt_requires_selected_server_rsa_key() {
        // 以 struct 更新語法設定欄位(避免 clippy field_reassign_with_default)
        let aux = launcher::server_list::AuxConfig {
            packet_encrypt: true,
            ..Default::default()
        };

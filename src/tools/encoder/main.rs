...
use launcher::dynamic_icon_format::{
    build_sprite_pak, find_entry, parse_dynamic_icons, to_xml, AnimEntry, AnimMap,
};
...
use launcher::server_list::{
    ...
    DEFAULT_IMG_LIMIT_VALUE, DEFAULT_INVENTORY_LIMIT_VALUE, DEFAULT_MULTI_INSTANCE_LIMIT,
    MAX_IMG_LIMIT_VALUE, MAX_INVENTORY_LIMIT_VALUE, MAX_MULTI_INSTANCE_LIMIT, MAX_SERVERS,
    MIN_IMG_LIMIT_VALUE, MIN_INVENTORY_LIMIT_VALUE, MIN_MULTI_INSTANCE_LIMIT,
};
...
/// 動態道具圖編輯器一筆條目（GUI 暫存：tbt + 時間 + 選好的 PNG 幀檔路徑）。
#[derive(Debug, Clone, Default)]
struct IconEntry {
    tbt: u16,
    speed: u16,
    interval: u32,
    /// 每幀的 PNG bytes（pick 時即讀入；可來自選檔或從舊 pak 還原）。
    frames: Vec<Vec<u8>>,
}
...
mod tests {
    ...
    fn encoder_aux_options_lock_anticheat_before_export() {
        ...
        assert!(locked.anti_cheat_basic); // 防外掛(視窗改名)為真功能,匯出時保留
        assert!(!locked.anti_cheat_advanced); // 進階防外掛仍鎖定關閉
    ...
    #[test]
    fn encoder_dynamic_icon_option_is_grouped_with_left_feature_toggles() {
        let source = include_str!("main.rs");
        assert!(
            source.contains("position: (15, 315), size: (150, 22),\r\n                  background_color: Some([255, 255, 255]))]\r\n    cb_dynamic_icon")
                || source.contains("position: (15, 315), size: (150, 22),\n                  background_color: Some([255, 255, 255]))]\n    cb_dynamic_icon"),
            "dynamic item icon option should sit directly below the left feature toggle group"
        );
        assert!(
            source.contains("position: (15, 340), size: (150, 22),\r\n                  background_color: Some([255, 255, 255]))]\r\n    cb_ac_mr_limit")
                || source.contains("position: (15, 340), size: (150, 22),\n                  background_color: Some([255, 255, 255]))]\n    cb_ac_mr_limit"),
            "AC/MR option should remain directly below dynamic item icon"
        );
    }
...
pub struct EncoderApp {
    ...
    #[nwg_control(parent: window, position: (5, 5), size: (500, 415))]
    ...
    #[nwg_control(parent: tab_encode, text: "動態道具圖",
                  position: (15, 315), size: (150, 22),
                  background_color: Some([255, 255, 255]))]
    cb_dynamic_icon: nwg::CheckBox,

    #[nwg_control(parent: tab_encode, text: "AC/MR突破上限",
                  position: (15, 340), size: (150, 22),
                  background_color: Some([255, 255, 255]))]
    cb_ac_mr_limit: nwg::CheckBox,

    #[nwg_control(parent: tab_encode, text: "允許多開",
                  position: (260, 198), size: (95, 22),
                  background_color: Some([255, 255, 255]))]
    cb_allow_multi: nwg::CheckBox,

    #[nwg_control(parent: tab_encode, text: "0",
                  position: (360, 196), size: (50, 24))]
    multi_limit_input: nwg::TextInput,

    #[nwg_control(parent: tab_encode, text: "0=無限",
                  position: (415, 200), size: (70, 22),
                  background_color: Some([255, 255, 255]))]
    multi_limit_hint: nwg::Label,

    // ─── 下方：Server 槽位選擇 + 編碼 ───
    #[nwg_control(parent: tab_encode,
                  position: (260, 228), size: (130, 26),
    ...
    // ════════════════ Tab：動態道具圖 ════════════════
    #[nwg_control(parent: tabs, text: "動態道具圖")]
    tab_icon: nwg::Tab,

    #[nwg_control(parent: tab_icon, text: "pak 名",
                  position: (10, 14), size: (50, 20),
                  background_color: Some([255, 255, 255]))]
    icon_pak_label: nwg::Label,
    #[nwg_control(parent: tab_icon, text: "123", position: (65, 12), size: (90, 24))]
    icon_pak_input: nwg::TextInput,

    #[nwg_control(parent: tab_icon, text: "PNG 起始編號",
                  position: (170, 14), size: (85, 20),
                  background_color: Some([255, 255, 255]))]
    icon_base_label: nwg::Label,
    #[nwg_control(parent: tab_icon, text: "30000", position: (255, 12), size: (80, 24))]
    icon_base_input: nwg::TextInput,

    #[nwg_control(parent: tab_icon, text: "開啟 pak…", position: (345, 11), size: (95, 26))]
    #[nwg_events(OnButtonClick: [EncoderApp::on_icon_open_pak])]
    icon_open_btn: nwg::Button,

    #[nwg_control(parent: tab_icon, text: "目標 gfxid",
                  position: (10, 48), size: (65, 20),
                  background_color: Some([255, 255, 255]))]
    icon_tbt_label: nwg::Label,
    #[nwg_control(parent: tab_icon, text: "", position: (78, 46), size: (60, 24))]
    icon_tbt_input: nwg::TextInput,

    #[nwg_control(parent: tab_icon, text: "速度ms",
                  position: (148, 48), size: (45, 20),
                  background_color: Some([255, 255, 255]))]
    icon_speed_label: nwg::Label,
    #[nwg_control(parent: tab_icon, text: "80", position: (195, 46), size: (50, 24))]
    icon_speed_input: nwg::TextInput,

    #[nwg_control(parent: tab_icon, text: "間隔ms",
                  position: (255, 48), size: (45, 20),
                  background_color: Some([255, 255, 255]))]
    icon_interval_label: nwg::Label,
    #[nwg_control(parent: tab_icon, text: "2000", position: (302, 46), size: (60, 24))]
    icon_interval_input: nwg::TextInput,

    #[nwg_control(parent: tab_icon, text: "選 PNG 幀…", position: (10, 78), size: (90, 26))]
    #[nwg_events(OnButtonClick: [EncoderApp::on_icon_pick_frames])]
    icon_pick_btn: nwg::Button,
    #[nwg_control(parent: tab_icon, text: "未選擇幀",
                  position: (108, 82), size: (370, 20),
                  background_color: Some([255, 255, 255]))]
    icon_frames_label: nwg::Label,

    #[nwg_control(parent: tab_icon, text: "加入條目", position: (10, 110), size: (90, 26))]
    #[nwg_events(OnButtonClick: [EncoderApp::on_icon_add_entry])]
    icon_add_btn: nwg::Button,
    #[nwg_control(parent: tab_icon, text: "移除選取", position: (108, 110), size: (90, 26))]
    #[nwg_events(OnButtonClick: [EncoderApp::on_icon_remove_entry])]
    icon_remove_btn: nwg::Button,

    #[nwg_control(parent: tab_icon, position: (10, 144), size: (470, 135))]
    icon_list: nwg::ListBox<String>,

    #[nwg_control(parent: tab_icon, text: "打包輸出…", position: (10, 286), size: (110, 30))]
    #[nwg_events(OnButtonClick: [EncoderApp::on_icon_pack])]
    icon_pack_btn: nwg::Button,
    #[nwg_control(parent: tab_icon,
                  text: "選輸出資料夾(放遊戲目錄)→產生 <pak名>.pak/.idx + dynamicicons.xml",
                  position: (128, 292), size: (352, 20),
                  background_color: Some([255, 255, 255]))]
    icon_pack_note: nwg::Label,
    ...
    /// 動態道具圖：已加入的條目
    icon_entries: RefCell<Vec<IconEntry>>,
    /// 動態道具圖：當前「選 PNG 幀」讀入、尚未加入條目的 PNG bytes
    icon_pending_frames: RefCell<Vec<Vec<u8>>>,
}
...
impl EncoderApp {
    ...
    fn collect_aux(&self) -> AuxConfig {
        normalize_encoder_aux_options(AuxConfig {
            ...
            multi_instance: self.is_checked(&self.cb_allow_multi),
            multi_instance_limit: Self::parse_limit_input(
                &self.multi_limit_input,
                DEFAULT_MULTI_INSTANCE_LIMIT,
                MIN_MULTI_INSTANCE_LIMIT,
                MAX_MULTI_INSTANCE_LIMIT,
            ),
            ...
            ac_mr_limit_enabled: self.is_checked(&self.cb_ac_mr_limit),
            ...
            dynamic_icon_enabled: self.is_checked(&self.cb_dynamic_icon),
            ...
            dynamic_icon_pak_name: {
                let n = self.icon_pak_input.text().trim().to_string();
                if n.is_empty() {
                    "123".to_string()
                } else {
                    n
    ...
    fn apply_aux(&self, a: &AuxConfig) {
        ...
        *self.aux_config.borrow_mut() = a.clone();
        ...
        self.set_checked(&self.cb_ac_mr_limit, a.ac_mr_limit_enabled);
        ...
        self.set_checked(&self.cb_dynamic_icon, a.dynamic_icon_enabled);
        self.icon_pak_input.set_text(&a.dynamic_icon_pak_name);
        self.set_checked(&self.cb_allow_multi, a.multi_instance);
        self.multi_limit_input
            .set_text(&a.multi_instance_limit.to_string());
        self.inventory_limit_input
    ...
    fn on_load(&self) {
        ...
        if cfg_path.exists() {
            if let Ok(raw) = launcher::legacy_text::read_text_file(&cfg_path) {
                match decrypt_config_text(&raw).and_then(|plain| parse_list_file(&plain)) {
                    Ok(file) => {
                        aux_loaded = file.aux;
    ...
    fn on_icon_open_pak(&self) {
        let mut dialog = Default::default();
        if nwg::FileDialog::builder()
            .title("開啟已打包的 pak（需同名 .idx 在旁）")
            .action(nwg::FileDialogAction::Open)
            .filters("pak(*.pak)|所有檔案(*.*)")
            .build(&mut dialog)
            .is_err()
        ...
        let pak_path = match dialog.get_selected_item() {
            Ok(p) => PathBuf::from(p),
            Err(_) => return,
        };
        let idx_path = pak_path.with_extension("idx");
        let pak = match std::fs::read(&pak_path) {
            Ok(b) => b,
            Err(e) => {
                self.alert(&format!("讀 pak 失敗：{e}"));
                return;
            }
        };
        let idx = match std::fs::read(&idx_path) {
            ...
            Err(e) => {
                self.alert(&format!(
                    "讀 idx 失敗（需 {} 旁有同名 .idx）：{e}",
                    pak_path.display()
                ));
                return;
            }
        };
        // 抽 dynamicicons.xml → 解析
        let xml_e = match find_entry(&idx, "dynamicicons.xml") {
            Some(e) => e,
            None => {
                self.alert("pak 內找不到 dynamicicons.xml");
                return;
            }
        };
        let xml_bytes = match pak.get(xml_e.offset as usize..(xml_e.offset + xml_e.size) as usize) {
            Some(b) => b,
            None => {
                self.alert("dynamicicons.xml 範圍超出 pak");
                return;
            }
        };
        let xml = match std::str::from_utf8(xml_bytes) {
            Ok(s) => s,
            Err(_) => {
                self.alert("dynamicicons.xml 非 UTF-8");
                return;
            }
        };
        let map = match parse_dynamic_icons(xml) {
            Ok(m) => m,
            Err(e) => {
                self.alert(&format!("解析 dynamicicons.xml 失敗：{e}"));
                return;
            }
        };
        // 還原每個條目的 PNG bytes（依 frames 的 png id 從 pak 抽）
        ...
        for e in map.values() {
            ...
            for &id in &e.frames {
                ...
                let fe = match find_entry(&idx, &format!("{id}.png")) {
                    Some(fe) => fe,
                    None => {
                        self.alert(&format!("pak 內缺 {id}.png（gfxid={}）", e.tbt));
                        return;
                    }
                };
                match pak.get(fe.offset as usize..(fe.offset + fe.size) as usize) {
                    Some(b) => frames.push(b.to_vec()),
                    None => {
                        self.alert(&format!("{id}.png 範圍超出 pak"));
                        return;
                    }
                }
            }
        ...
        let n = entries.len();
        *self.icon_entries.borrow_mut() = entries;
        if min_id != u32::MAX {
            self.icon_base_input.set_text(&min_id.to_string());
        }
        if let Some(stem) = pak_path.file_stem() {
            self.icon_pak_input.set_text(&stem.to_string_lossy());
        }
        self.refresh_icon_list();
        self.alert(&format!("已載入 {n} 筆條目，可繼續編輯後重新打包。"));
    }

    fn on_icon_pick_frames(&self) {
        let mut dialog = Default::default();
        if nwg::FileDialog::builder()
            .title("選擇 PNG 幀（可多選，順序即播放順序）")
            .action(nwg::FileDialogAction::Open)
            .multiselect(true)
            .filters("PNG(*.png)|所有檔案(*.*)")
            .build(&mut dialog)
            .is_ok()
            && dialog.run(Some(&self.window))
        {
            if let Ok(items) = dialog.get_selected_items() {
                let mut frames: Vec<Vec<u8>> = Vec::new();
                for p in &items {
                    match std::fs::read(p) {
                        Ok(b) => frames.push(b),
                        Err(e) => {
                            self.alert(&format!("讀 PNG 失敗：{}\n{e}", Path::new(p).display()));
                            return;
                        }
                    }
                }
                self.icon_frames_label
                    .set_text(&format!("已選 {} 幀", frames.len()));
                *self.icon_pending_frames.borrow_mut() = frames;
            }
        }
    }

    fn on_icon_add_entry(&self) {
        let frames = self.icon_pending_frames.borrow().clone();
        if frames.is_empty() {
            self.alert("請先「選 PNG 幀」");
            return;
        }
        if frames.len() > 99 {
            self.alert(&format!("一個條目最多 99 幀（目前 {}）", frames.len()));
            return;
        }
        let tbt = match self.icon_tbt_input.text().trim().parse::<u16>() {
            Ok(v) if v > 0 => v,
            _ => {
                self.alert("目標 gfxid 需為 1~65535 的數字");
                return;
            }
        };
        let speed = self
        ...
        let interval = self
            ...
            .unwrap_or(0);
        {
            let mut entries = self.icon_entries.borrow_mut();
            entries.retain(|e| e.tbt != tbt); // 同 tbt → 取代
            entries.push(IconEntry {
                tbt,
                speed,
                interval,
        ...
        self.icon_pending_frames.borrow_mut().clear();
        self.icon_frames_label.set_text("未選擇幀");
        self.refresh_icon_list();
    }

    fn on_icon_remove_entry(&self) {
        match self.icon_list.selection() {
            Some(sel) => {
                {
                    let mut entries = self.icon_entries.borrow_mut();
                    if sel < entries.len() {
            ...
            None => self.alert("先在清單選一個條目"),
        }
    }

    fn refresh_icon_list(&self) {
        let items: Vec<String> = self
            ...
            .collect();
        self.icon_list.set_collection(items);
    }

    fn on_icon_pack(&self) {
        if self.icon_entries.borrow().is_empty() {
            self.alert("沒有條目可打包");
            return;
        }
        let pak_name = self.icon_pak_input.text().trim().to_string();
        if pak_name.is_empty() {
            self.alert("pak 名不可空白");
            return;
        }
        let base: u32 = match self.icon_base_input.text().trim().parse() {
            Ok(v) => v,
            Err(_) => {
                self.alert("PNG 起始編號需為數字");
                return;
            }
        };
        // 選輸出資料夾（放遊戲目錄）
        ...
        let out_dir = match dialog.get_selected_item() {
            Ok(p) => PathBuf::from(p),
            Err(_) => return,
        };

        // 組 AnimMap + 收集檔案（png id 從 base 連號）
        let mut map = AnimMap::new();
        let mut files: Vec<(String, Vec<u8>)> = Vec::new();
        let mut next_id = base;
        for e in self.icon_entries.borrow().iter() {
            let mut frame_ids = Vec::new();
            for body in &e.frames {
                files.push((format!("{next_id}.png"), body.clone()));
                frame_ids.push(next_id);
                next_id += 1;
            }
            map.insert(
                e.tbt,
        ...
        let xml = to_xml(&map);
        files.insert(0, ("dynamicicons.xml".to_string(), xml.into_bytes()));
        let (pak, idx) = match build_sprite_pak(&files) {
            Ok(v) => v,
            Err(err) => {
                self.alert(&format!("打包失敗：{err}"));
                return;
            }
        };
        let pak_path = out_dir.join(format!("{pak_name}.pak"));
        let idx_path = out_dir.join(format!("{pak_name}.idx"));
        if let Err(err) = std::fs::write(&pak_path, &pak) {
            self.alert(&format!("寫 {} 失敗：{err}", pak_path.display()));
            return;
        }
        if let Err(err) = std::fs::write(&idx_path, &idx) {
            self.alert(&format!("寫 {} 失敗：{err}", idx_path.display()));
            return;
        }
        self.alert(&format!(
            "打包完成\n\n輸出：\n{}\n{}\n\n條目：{} 筆，PNG：{} 張（編號 {}~{}）\n\n\
             記得 launcher 設定 dynamic_icon_enabled=true、dynamic_icon_pak_name={}",
            pak_path.display(),
            idx_path.display(),
            self.icon_entries.borrow().len(),
            next_id - base,
            base,
    ...
    fn scan_skins(&self) -> Vec<String> {
        ...
        if let Ok(entries) = std::fs::read_dir(&dir) {
            for entry in entries.flatten() {
                if entry.path().is_dir() && entry.path().join("index.html").exists() {

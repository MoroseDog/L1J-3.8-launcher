...
pub fn install(h: HANDLE, pid: u32) -> Result<()> {
    ...
    let game_hwnd = crate::patching::img_hover::find_hwnd_by_pid(pid)
...
fn get_apis() -> Result<(u32, u32, u32, u32)> {
    ...
    unsafe {
        ...
        // 函式指標 → 位址(i686 上 usize == u32):先轉 usize 取完整位址再窄化成
        // shellcode 用的 u32,避免 clippy fn_to_numeric_cast(直接轉數值型別)警告。
        Ok((
            peek as usize as u32,
            fg as usize as u32,
            sleep as usize as u32,
            get_async_key_state as usize as u32,
        ))
    }
}

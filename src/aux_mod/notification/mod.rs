    ...
    match *state {
        ...
        Some((last_at, last_total, last_pkt)) => {
            if now.duration_since(last_at) >= PROBE_INTERVAL {
                ...
                // overlay 視窗為現行渲染路徑(不碰遊戲記憶體);舊 codecave 繪圖 diag 已隨
                // image_draw_hook / renderer_install 一同移除,這裡只保留 polling 健康度。
                log_line!("[notification] render diag: polling_ticks={ticks} fails={fails}");

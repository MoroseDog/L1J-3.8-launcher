...
impl DrinkHandle {
    ...
    /// `target_mode` 控制 send path:
    /// - [`SkillTargetMode::NoSpec`]   (`/M`)  — 完全不動 `[0x97C910]`,讓 dispatcher 用當下值
    /// - [`SkillTargetMode::ForceSelfPacket`] — bypass spell_book_cast 直送 self-cast packet
    /// - [`SkillTargetMode::ForceTargetPacket(id)`] — bypass spell_book_cast 直送 target packet
    ///
...
///
/// TARGET SETUP 段:
/// - [`SkillTargetMode::NoSpec`]   — 0 bytes(直接跳過,不動 `[TARGET]`)
///
/// 其餘 mode(ForceSelfPacket / ForceTargetPacket)在函數開頭就分流到
/// 各自的 packet shellcode,不會走到下面的共通骨架。
...
mod tests {
    ...
    fn skill_shellcode_common_tail() {
        // NoSpec 的 tail(restore target + popad + ret)結構:
        // 最後 8 bytes:[58 A3 <TARGET 4B> 61 C3]
        // ForceSelfPacket / ForceTargetPacket 走各自 packet 路徑,排除。
        // 刪除其他 variant 後只剩 NoSpec 一種需驗 tail,直接驗不用迴圈
        let sc = build_skill_shellcode(42, SkillTargetMode::NoSpec);
        let n = sc.len();

...
pub const DEFAULT_MULTI_INSTANCE_LIMIT: u32 = 0;
pub const MIN_MULTI_INSTANCE_LIMIT: u32 = 0;
pub const MAX_MULTI_INSTANCE_LIMIT: u32 = 32;
...
/// configenc.cpp 固定 XOR 表（與 tools/server_config.py、src/platform/inject.rs 同一份）
...
pub struct AuxConfig {
    ...
    pub ac_mr_limit_enabled: bool,     // AC/MR 突破 byte 上限
    ...
    pub dynamic_icon_enabled: bool,    // 動態道具圖(自製 pak PNG 動畫)
    pub pickup_toast_enabled: bool,    // 左下道具提示(撿物 toast)
    pub exp_drift_enabled: bool,       // 金幣經驗值提示(畫面飄字)
    pub multi_instance_limit: u32,     // 0=無限多開, N=最多 N 開(防外掛)
    pub inventory_limit_value: u32,
    pub img_limit_value: u32,
    pub dynamic_icon_pak_name: String, // 動態道具圖自製 pak 基底名(如 "123" → 123.pak/123.idx)
...
fn normalize_aux_config(mut aux: AuxConfig) -> AuxConfig {
    aux.anti_cheat_advanced = false; // 進階防外掛尚未實作,強制關閉
    aux
}
            ...
            "aux" => {
                ...
                match key.as_str() {
                    ...
                    "multi_instance_limit" => {
                        aux.multi_instance_limit = parse_u32_clamped(
                            value,
                    ...
                    "ac_mr_limit_enabled" => aux.ac_mr_limit_enabled = v,
                    ...
                    "dynamic_icon_enabled" => aux.dynamic_icon_enabled = v,
                    ...
                    "dynamic_icon_pak_name" => {
                        let name = value.trim();
                        if !name.is_empty() {
                            aux.dynamic_icon_pak_name = name.to_string();
                        }
                    }
...
pub fn build_list_file(file: &ListFile) -> Result<String> {
    ...
    out.push_str(&format!("ac_mr_limit_enabled={}\n", a.ac_mr_limit_enabled));
    out.push_str(&format!(
    ...
    out.push_str(&format!(
        "dynamic_icon_enabled={}\n",
        a.dynamic_icon_enabled
    ));
    out.push_str(&format!(
        "dynamic_icon_pak_name={}\n",
        a.dynamic_icon_pak_name
    ));
    out.push_str("\n[launcher]\n");
    let l = &file.launcher;
...
mod tests {
    ...
    fn list_file_with_aux_roundtrip() {
        ...
        assert!(!txt.contains("classic_item_tooltip_enabled"));
    ...
    #[test]
    fn unknown_aux_key_is_ignored_and_not_serialized() {
        let parsed =
            parse_list_file("[aux]\nlegacy_removed_key=true\nlhx_aux_enabled=false\n").unwrap();
        assert!(!parsed.aux.lhx_aux_enabled);

        let serialized = build_list_file(&parsed).unwrap();
        assert!(!serialized.contains("legacy_removed_key"));
        assert!(serialized.contains("lhx_aux_enabled=false"));
    }
    ...
    #[test]
    fn anti_cheat_basic_is_kept_advanced_is_forced_off() {
        let parsed = parse_list_file(
            "[aux]\nanti_cheat_basic=true\nanti_cheat_advanced=true\npacket_encrypt=true\n",
        )
        .unwrap();

        assert!(parsed.aux.anti_cheat_basic); // 防外掛=視窗改名,真功能,保留
        assert!(!parsed.aux.anti_cheat_advanced); // 進階防外掛仍未實作,強制關閉
        assert!(parsed.aux.packet_encrypt);
    }

    #[test]
    fn multi_instance_limit_roundtrips_and_defaults_to_zero() {
        // 舊 config 缺欄位 → 預設 0(無限)
        let legacy = parse_list_file("[aux]\nmulti_instance=true\n").unwrap();
        assert_eq!(legacy.aux.multi_instance_limit, 0);
        ...
        let txt = build_list_file(&original).unwrap();
        assert!(txt.contains("multi_instance_limit=3"));
        let parsed = parse_list_file(&txt).unwrap();
        assert_eq!(parsed.aux.multi_instance_limit, 3);

        // 超過 MAX 會被夾住
        let clamped = parse_list_file("[aux]\nmulti_instance_limit=999\n").unwrap();
        assert_eq!(clamped.aux.multi_instance_limit, MAX_MULTI_INSTANCE_LIMIT);
    }

    #[test]
    fn ac_mr_limit_enabled_roundtrips_and_defaults_true() {
        // 預設 true
        assert!(AuxConfig::default().ac_mr_limit_enabled);

        // 舊 config 缺欄位 → 預設 true(向後相容)
        let legacy = parse_list_file("[aux]\npacket_encrypt=true\n").unwrap();
        assert!(legacy.aux.ac_mr_limit_enabled);

        // build → parse 保值(設 false)
        let aux = AuxConfig {
            ac_mr_limit_enabled: false,
            ..Default::default()
        };
        let original = ListFile {
            servers: vec![],
        ...
        let txt = build_list_file(&original).unwrap();
        assert!(txt.contains("ac_mr_limit_enabled=false"));
        let parsed = parse_list_file(&txt).unwrap();
        assert!(!parsed.aux.ac_mr_limit_enabled);
    }
    ...
    fn config_ini_full_pipeline() {
        ...
        assert!(parsed.aux.packet_encrypt);
        assert!(parsed.aux.transform_file);
        assert!(parsed.aux.multi_instance);
        assert_eq!(parsed.aux.lhx_aux_enabled, aux.lhx_aux_enabled);
    }
}

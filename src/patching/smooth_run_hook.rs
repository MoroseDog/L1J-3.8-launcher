...
use crate::aux::address::player_offset::{ANIM_FRAME, HASTE_HIGH};
use crate::logger::log_line;
use crate::platform::{memory, process};
use crate::smooth_run::types::RunSlot;
use anyhow::Result;
...
// RunL / RunR slot 的 frame_data 偏移(`slot*8+4`)由 RunSlot 提供:
// RunSlot::Left.frame_data_off() = 0x314(98*8+4)、RunSlot::Right = 0x31C(99*8+4)。
// 加速欄位 entity+0x29 與 anim_frame entity+0x17 直接引用 HASTE_HIGH / ANIM_FRAME
// (aux::address::player_offset 正典),不再本地重定義。
...
pub fn install_smooth_run_hook(h: HANDLE, pid: u32) -> Result<()> {
    ...
    let cave_size = (HASH_TABLE_OFF as usize) + HASH_TABLE_SIZE * 4;
...
fn build_shellcode(cave_addr: u32) -> Vec<u8> {
    ...
    sc.extend_from_slice(&RunSlot::Left.frame_data_off().to_le_bytes());
    ...
    sc.extend_from_slice(&[0x80, 0x79, HASTE_HIGH as u8, 0x00]);
    ...
    sc.extend_from_slice(&[0x0F, 0xB6, 0x41, ANIM_FRAME as u8]);
    ...
    sc.extend_from_slice(&[0x81, 0xBA]);
    sc.extend_from_slice(&RunSlot::Right.frame_data_off().to_le_bytes());
    sc.extend_from_slice(&0x00010000u32.to_le_bytes());
...
mod tests {
    ...
    #[test]
    fn shellcode_fits_before_hash_table() {
        let sc = build_shellcode(0x1234_0000);
        assert!(
            sc.len() <= HASH_TABLE_OFF as usize,
            "shellcode ({}) 不可覆蓋 hash table 偏移 ({})",
            sc.len(),
            HASH_TABLE_OFF
        );
    }

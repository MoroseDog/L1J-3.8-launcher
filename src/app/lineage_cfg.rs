...
const DEFAULT_WINDOW_MODE: u32 = WindowMode::DEFAULT.as_raw() as u32;
...
/// 把 WindowMode 改成 mode。合法性由 [`WindowMode`] 型別保證,不再需要執行期範圍檢查。
pub fn set_window_mode(game_dir: &str, mode: WindowMode) -> Result<()> {
    let raw = mode.as_raw() as u32;
    update_value(game_dir, KEY_WINDOW_MODE, &raw.to_le_bytes(), "WindowMode")
}
...
mod tests {
    ...
    fn set_window_mode_updates_cfg_dword() {
        ...
        set_window_mode(dir.to_str().unwrap(), WindowMode::W1600x1200).unwrap();

...
pub fn load_png_paired(black_name: &str, white_name: &str) -> Option<DecodedPng> {
    ...
    for i in 0..pixels {
        ...
        // straight color = premultiplied black / (alpha/255);alpha==0 時 checked_div 回 None
        // → unwrap_or(0),與原本 `if alpha==0 {(0,0,0)}` 守衛逐通道等價(消 divide-by-zero 分支)。
        let unpremul = |c: i32| {
            (c as u32 * 255 + alpha / 2)
                .checked_div(alpha)
                .unwrap_or(0)
                .min(255) as u8
        };
        let (rr, gg, bb_out) = (unpremul(br), unpremul(bg), unpremul(bb));
        rgba.push(rr);
        rgba.push(gg);
        rgba.push(bb_out);

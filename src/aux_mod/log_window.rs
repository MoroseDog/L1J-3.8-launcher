...
pub fn spawn_log_window_thread(visible: Arc<AtomicU8>) -> JoinHandle<()> {
    ...
    std::thread::spawn(move || {
        ...
        // Keep this window independent. Attaching helper windows to the game
        // HWND can resize the Lineage host window while the backbuffer stays
        // 800x600, which leaves black regions and duplicated UI.

...
pub(super) fn parse_variant_line(trimmed: &str) -> Option<(&str, u32, u32, &str)> {
    let mut num_end = 0;
...
pub(super) fn extract_action_name(trimmed: &str) -> &str {
    // 找到 "N." 之後、"(" 之前的部分
...
pub(super) fn extract_inline_action_with_name(
    line: &str,
    target_action: u32,
) -> Option<(&str, &str)> {

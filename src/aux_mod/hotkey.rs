...
static HOTKEY_SETTINGS: Mutex<Option<Arc<RwLock<AuxSettings>>>> = Mutex::new(None);
...
pub fn install(
    ...
    *HOTKEY_SETTINGS.lock() = Some(settings.clone());
...
unsafe extern "system" fn low_level_kb_proc(code: i32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    if code >= 0 {
        ...
        if msg_id == WM_KEYDOWN || msg_id == WM_SYSKEYDOWN {
            ...
            if let Some(idx) = idx {
                if game_focused() && current_fkey_macro_should_intercept(idx) {
...
fn current_fkey_macro_should_intercept(idx: usize) -> bool {
    let settings = HOTKEY_SETTINGS.lock().as_ref().cloned();
    let Some(settings) = settings else {
        return false;
    };
    let settings = settings.read();
    fkey_macro_should_intercept(&settings, idx)
}

fn fkey_macro_should_intercept(settings: &AuxSettings, idx: usize) -> bool {
    match settings.fkey_macros.get(idx) {
        Some(m) => m.enabled && !m.command.trim().is_empty(),
        None => false,
    }
}
...
mod tests {
    ...
    #[test]
    fn fkey_intercept_respects_each_enabled_slot() {
        let mut s = crate::aux::runtime::AuxSettings::default();
        s.fkey_macros[0].enabled = true;
        s.fkey_macros[0].command = "red potion".to_string();
        s.fkey_macros[1].command = "orange potion".to_string();

        assert!(super::fkey_macro_should_intercept(&s, 0));
        assert!(!super::fkey_macro_should_intercept(&s, 1));
        assert!(!super::fkey_macro_should_intercept(&s, 2));
        assert!(!super::fkey_macro_should_intercept(&s, 3));
    }

    #[test]
    fn fkey_intercept_requires_a_command() {
        let mut s = crate::aux::runtime::AuxSettings::default();
        s.fkey_macros[0].enabled = true;
        s.fkey_macros[0].command = "   ".to_string();
